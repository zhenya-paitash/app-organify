...
class AppwriteDropper {
   ...
   * Delete the database
   */
  async deleteDatabase(name: string = 'OrganifyDB'): Promise<void> {
    if (!this.databases) {
      throw new Error('Databases service not initialized');
    }
   ...
   */
  async deleteBucket(name: string = 'OrganifyImages'): Promise<void> {
    if (!this.storage) {
      throw new Error('Storage service not initialized');
    }
  ...
  async deleteAllUsers(): Promise<void> {
    ...
    try {
      ...
      const confirmation = await this.rl.question(
        'WARNING: This will delete ALL users in your project. Type "DELETE" to confirm: '
      );
  ...
  async dropEverything(): Promise<void> {
    try {
      ...
      // Confirm action
      const confirmation = await this.rl.question(
        'WARNING: This will delete ALL OrganifyDB data, images, and users. This action cannot be undone.\n' +
      ...
      // Delete database
      await this.deleteDatabase('OrganifyDB');

      // Delete bucket
      await this.deleteBucket('OrganifyImages');
